...
def get_np_from_npy(file_dir, flag_crop=True):
    ...
    if flag_crop: # 3/4的概率使用截取方式一
        ...
        image_np_ex = get_crop_from_np(img_np=image_np, out_size=(64, 128, 128)) # 更改尺寸

    else: # 1/4的概率使用截取方式一
        image_np = scipy.ndimage.zoom(input=image_np, zoom=[f_scale_0, f_scale_1, f_scale_1]) # 一样，进行值的缩放
        image_np_ex = get_crop_from_np_ct(img_np=image_np, out_size=(64, 128, 128)) # 更改尺寸
...
if __name__=='__main__':
    ...
    # 指定模型保存路径
    model_dir = 'test5_128_32_3D_res_pic.pb'
    ...
    if train_flag:
        ...
        result_dir = './ret2_' + str(date.year) + '-' + str(date.month) + '-' + str(date.day) + '-' + str(
            date.hour) + '.csv'
